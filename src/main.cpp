...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "RPG", NULL, NULL);
  if (window == NULL) {
    // TODO: log error correctly and insted of null to get error give string
    // array.
    std::cout << "Error creating window " << std::hex << glfwGetError(NULL)
              << std::endl;
    glfwTerminate();
    return -1;
  }
